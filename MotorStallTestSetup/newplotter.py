...
def load_and_run(dict: dict, stalled_filename, normal_filename):
    x_data, stalled_data, normal_data = [], {}, {}
    for key, value in dict:
        ...
        with open(stalled_filename, 'r', newline='') as f:
            ...
            for row in reader:
                try:
                    ...
                    current = float(row[value])  # convert to C

                    x_data.append(t)
                    stalled_pair.append(current)
        ...
        with open(normal_filename, 'r', newline='') as f:
            ...
            for row in reader:
                try:
                    t = float(row[0])      # time (µs or ms)
                    current = float(row[value])  # convert to current

                    normal_pair.append(current)

                except:
                    pass
        stalled_data[key] = stalled_pair
        normal_data[key] = normal_pair
