...
#define SAMPLE_US  1000    // ~1 kHz output
#define OVERSAMPLE 7       // median of N fast reads per output sample (rejects brush-commutation spikes)

static const int ADC_MAX = (1 << ADC_BITS) - 1;

// Median of OVERSAMPLE back-to-back ADC reads. Brushed-motor commutation produces brief
// voltage spikes; a median rejects them where a plain average would still be pulled up.
int readMedian() {
  int v[OVERSAMPLE];
  for (int i = 0; i < OVERSAMPLE; i++) v[i] = analogRead(ADC_PIN);
  for (int i = 1; i < OVERSAMPLE; i++) {     // insertion sort (N is tiny)
    int k = v[i], j = i - 1;
...
void loop() {
  ...
  if ((long)(now - next) >= 0) {
    next = now + SAMPLE_US;
    int raw = readMedian();           // median-filtered sample (despikes commutation noise)
